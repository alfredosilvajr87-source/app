...
const ReportsPage = () => {
  ...
  const [stockStatusFilter, setStockStatusFilter] = useState('all'); // all | critical | low | ok
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm"
                onClick={() => exportToCsv(
                  stockStatusFilter === 'all' ? stockStatus : stockStatus.filter(i => i.status === stockStatusFilter),
                  `stock_${stockStatusFilter}`
                )}
              ...
              <div className="overflow-x-auto">
                <table className="data-table">
                  ...
                  <tbody>
                    {(stockStatusFilter === 'all'
                      ? stockStatus
                      : stockStatus.filter(i => i.status === stockStatusFilter)
                    ).map((item) => (
                      <tr key={item.item_id}>
