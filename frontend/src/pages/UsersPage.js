...
import { Users, Plus, Pencil, Trash2, Shield, User, Building2 } from 'lucide-react';
...
const UsersPage = () => {
  ...
  const toggleUnit = (unitId) => {
    setFormData(prev => {
      const ids = prev.unit_ids || [];
      return {
        ...prev,
        unit_ids: ids.includes(unitId) ? ids.filter(id => id !== unitId) : [...ids, unitId]
      };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = typeof detail === 'string' ? detail : Array.isArray(detail) ? detail.map(d => d.msg).join(', ') : 'Operation failed';
      toast.error(msg);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      const detail = err.response?.data?.detail;
      const msg = typeof detail === 'string' ? detail : Array.isArray(detail) ? detail.map(d => d.msg).join(', ') : 'Failed to delete user';
      toast.error(msg);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {units.length > 0 && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500">Leave all unchecked = access to all units</p>
                  ...
                  {units.map(unit => (
                    <div key={unit.id} className="flex items-center gap-2">
                      ...
                      <label htmlFor={`unit-${unit.id}`} className="text-sm cursor-pointer">
                        {unit.name} <span className="text-slate-400">({unit.initials})</span>
                      </label>
                    </div>
                  ))}
                </div>
                {(formData.unit_ids || []).length > 0 && (
                  <p className="text-xs text-amber-600">
                    This user will only see {formData.unit_ids.length} unit(s)
                  </p>
                )}
              </div>
            )}
